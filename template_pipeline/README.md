...
```bash
uv add --dev pgcli "psycopg[binary]"
```
...
> Note: `psycopg[binary]` is needed because pgcli depends on **psycopg v3**, which requires the PostgreSQL client library (`libpq`). The `[binary]` extra bundles libpq so you don't need to install it separately. This is a different package from `psycopg2-binary` (which is used by SQLAlchemy later).
